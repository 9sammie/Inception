...
Only NGINX has its ports exposed to the host system:
...
To modify a configuration file (like nginx.conf) and rebuild only that specific image without tearing down the entire stack:
	...
	docker compose up -d --build nginx# Inception - Developer & Architecture Documentation
...
### 5. Check Mariadb database
To access mariadb storage:

	docker exec -it mariadb mysql -u maballet -p
